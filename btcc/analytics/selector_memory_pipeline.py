...
from btcc.sim.selector_memory_config import CF_ARM_LABELS, MEMORY_ARM_LABELS
...
def _selection_regret(legs: pd.DataFrame, selection: pd.DataFrame) -> pd.DataFrame:
    ...
    best = cf.groupby("opportunity_id").agg(
        best_cf_pnl_pct=("pnl_pct", "max"),
        best_counterfactual_strategy=("arm_key", lambda s: s.loc[s.index[cf.loc[s.index, "pnl_pct"].idxmax()]] if len(s) else None),
    )
    rows = []
    ...
    for _, s in selection.iterrows():
        ...
        best_row = best.loc[oid] if oid in best.index else None
        best_v = float(best_row["best_cf_pnl_pct"]) if best_row is not None else realized
        best_arm = s.get("selected_arm_label")
        if best_row is not None:
            cf_opp = cf[cf["opportunity_id"] == oid]
            if not cf_opp.empty:
                idx = cf_opp["pnl_pct"].idxmax()
                best_arm = str(cf_opp.loc[idx, "arm_key"])
        rows.append({
            "opportunity_id": oid,
            "arm_label": arm,
...
def build_memory_analytics_asof(
    ...
    *,
    ...
    metrics = {arm: _arm_metrics(sel_legs[sel_legs["arm_key"] == arm], starting_capital_usd=starting_capital_usd) for arm in MEMORY_ARM_LABELS}
    ...
    daily_cap = capital_daily_series(cap_legs, eval_start=eval_start, starting_capital_usd=starting_capital_usd) if not cap_legs.empty else pd.DataFrame()
...
def _write_report(out_dir: Path, payload: dict, recommendation: dict) -> None:
    ...
    lines = [
        ...
        "",
        "## 1. Objective",
        "Compare Selector E lookback lengths (10–365 days) under strict walk-forward evaluation.",
        "",
        "## 2. Dataset period",
    ...
    lines += [
        ...
        "",
        "## 14. Limitations",
        "E-365 requires full warmup; common eval period shared across variants after warmup.",
        "",
