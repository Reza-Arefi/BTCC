...
_COLORS = plt.cm.viridis(np.linspace(0.1, 0.9, len(MEMORY_ARM_LABELS)))
...
def generate_memory_plots(
    ...
    *,
    ...
    if not daily_cap.empty:
        ...
        for i, arm in enumerate(MEMORY_ARM_LABELS):
            g = daily_cap[daily_cap["strategy_key"] == arm].sort_values("day_number")
            if g.empty:
                continue
            ret = 100.0 * (g["equity_usd"] / g["equity_usd"].iloc[0] - 1.0)
            ax.plot(g["day_number"], ret, label=arm, color=_COLORS[i], linewidth=2)
        ax.set_title("Cumulative return — E lookback variants")
        ax.set_xlabel("Day")
        ...
        for i, arm in enumerate(MEMORY_ARM_LABELS):
            ...
            ax.plot(g["day_number"], dd, label=arm, color=_COLORS[i], linewidth=2)
        ax.set_title("Drawdown — E lookback variants")
        ax.set_xlabel("Day")
        ...
        for i, arm in enumerate(MEMORY_ARM_LABELS):
            ...
            ax.plot(g["day_number"], ret, label=arm, color=_COLORS[i], alpha=0.85)
    ...
    vals = [float((metrics.get(a) or {}).get("avg_trade_return_pct") or 0.0) for a in MEMORY_ARM_LABELS]
    ax.bar(MEMORY_ARM_LABELS, vals, color=_COLORS)
    ...
    # 4. Profit factor
    fig, ax = plt.subplots(figsize=(8, 4))
    vals = [float((metrics.get(a) or {}).get("profit_factor") or 0.0) for a in MEMORY_ARM_LABELS]
    ax.bar(MEMORY_ARM_LABELS, vals, color=_COLORS)
    ...
    if not regret_df.empty:
        fig, ax = plt.subplots(figsize=(10, 5))
        data = [regret_df[regret_df["arm_label"] == a]["regret_pct"].dropna().values for a in MEMORY_ARM_LABELS]
        ax.boxplot(data, labels=MEMORY_ARM_LABELS)
    ...
    if regime:
        ...
        if mat and reg_keys:
            ...
            ax.set_title("Return % by regime × E version")
    ...
    if s_band:
        ...
        if mat and sb_keys:
            ...
            ax.set_yticks(range(len(arms)))
            ax.set_yticklabels(arms)
            ax.set_title("Avg trade return % by S-band × E version")
            fig.colorbar(im, ax=ax)
