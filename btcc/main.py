...
def main() -> int:
    ...
    if args.command == "run":
        ...
        sim_mode = (
            "Selector E paper trading"
            if getattr(engine, "selector_live_engine", None) is not None
            else "Adaptive V2 paper sim"
        )
        logger.info(
            "Entering 15m loop (SIGNAL ONLY + %s). service=%s last_candle=%s",
            sim_mode,
            under_systemd,
