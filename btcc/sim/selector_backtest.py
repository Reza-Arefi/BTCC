"""365-day selector experiment — fixed T1–T12 + dynamic selectors A–F.
...
def run_selector_experiment_backtest(
    *,
    ...
    if out_dir is None:
        ...
        out_dir = out_root / f"selector_compare_{days}d_thr0p60_{run_id}"
    ...
    fixed_specs = [spec_by_key[k] for k in FIXED_STRATEGY_KEYS]
        ...
        for base, coin in panels["coins"].items():
            ...
            if ep["trade_suggested"]:
                ...
                if future.empty:
                    ...
                    for arm_label, leg in zip(FIXED_ARM_LABELS, cf_legs):
                        leg_items.append({
                            "leg": leg,
                            "arm_key": arm_label,
                    ...
                    for sel in selectors.values():
                        pick = sel.select(cf_history, entry_ts, regime_info["regime"])
                        ...
                        selection_rows.append({
                            ...
                            **{f"score_{k}": pick["scores"].get(k) for k in FIXED_STRATEGY_KEYS},
                        })
    ...
    summary.update({
        ...
        "upper_threshold": hi,
        "n_arms": 18,
        "fixed_strategies": list(FIXED_ARM_LABELS),
    ...
    manifest = {
        "experiment_name": "selector_compare_365d_thr0p60",
        "experiment_kind": "selector_experiment_v1",
        "eval_start": str(eval_start),
        "eval_end": str(eval_end),
