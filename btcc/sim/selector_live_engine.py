...
from btcc.sim.selector_live_bootstrap import maybe_bootstrap_cf_history
...
from btcc.execution.factory import build_execution_stack
from btcc.execution.modes import ExecutionMode
from btcc.execution.risk import RiskContext
from btcc.execution.types import TradeIntent
...
class SelectorLiveEngine:
    ...
    def __init__(self, signal_cfg: dict[str, Any], sim_cfg: dict[str, Any], tg_send=None):
        ...
        if rt:
            ...
            maybe_bootstrap_cf_history(self.cf_history, sim_cfg)
        state = self.store.load_state()
        self.equity = PaperEquityTracker.from_state(sim_cfg, state.get("paper_equity") or {})
        ...
        # Stage 1 execution stack: PAPER OMS only. REAL is fail-closed and not constructed here.
        self.oms = build_execution_stack(
            sim_cfg,
            costs=self.costs,
            equity=self.equity,
            open_opps_provider=lambda: self.open_opps,
        )
        assert self.oms.mode == ExecutionMode.PAPER
        ...
        self._notify_hourly = bool(tg_cfg.get("send_hourly_summary", True))
        state0 = self.store.load_state()
        self._last_hourly_telegram_bucket: str | None = state0.get("last_hourly_telegram_bucket")
    ...
    def notify_startup(self) -> None:
        sl = self.sim.get("selector_live_raw") or {}
        sel = sl.get("selector") or {}
        banner = "\n".join([
            "=" * 40,
            "PAPER TRADING",
            f"Selector: E / {sel.get('kind', 'rank_ewma')}",
            f"Execution mode: {self.oms.mode.value}",
            f"Broker: {self.oms.broker.name}",
            f"Allocation: {self.equity.allocation_pct * 100:.0f}% of current equity",
            f"Max positions: {self.sim.get('max_open_opportunities', 10)}",
            "Max total exposure: 100%",
            "Leverage: NONE",
            "Real orders: DISABLED",
            f"ALLOW_TRADING: FALSE",
            f"Version: {self.version.get('selector_version')}",
            f"STARTING EQUITY: {self.equity.starting_equity_btc:.8f} BTC",
            f"CURRENT EQUITY: {self.equity.equity_btc:.8f} BTC",
            f"EWMA HALF-LIFE: {sel.get('half_life_days', 7)}d",
            f"CF WARMUP: {sl.get('cf_warmup_days', 10)}d ({len(self.cf_history.trades)} CF trades loaded)",
            f"ENTRY THRESHOLD: S >= {self.sm.long_threshold:.2f}",
            "=" * 40,
        ])
        logger.info("\n%s", banner)
        # Persist prospective sizing/version markers without altering equity or opens.
        state = self.store.load_state()
        state["paper_equity"] = self.equity.to_state()
        state["version"] = dict(self.version)
        self.store.save_state(state)
        self.trade_notifier.paper_startup(
            equity_btc=self.equity.equity_btc,
            allocation_pct=self.equity.allocation_pct,
            max_positions=int(self.sim.get("max_open_opportunities", 10)),
            cf_trades=len(self.cf_history.trades),
            version=self.version,
        )
        # Immediate digest on boot; then once per UTC hour after each cycle.
        self._maybe_send_hourly_summary()
    ...
    def _advance_open(self, rel_panels: dict[str, pd.DataFrame], btc_df: pd.DataFrame) -> list[dict]:
        ...
        for opp in self.open_opps:
            ...
            if all(it["leg"].closed for it in opp.get("leg_items") or []):
                ...
                if e_rec:
                    pnl_btc = float(e_rec.get("pnl_btc") or 0.0)
                    self.equity.apply_close(pnl_btc)
                    e_rec["equity_btc_after"] = self.equity.equity_btc
                    e_rec["account_impact_pct"] = (
                        100.0 * pnl_btc / self.equity.starting_equity_btc
                        if self.equity.starting_equity_btc else 0.0
                    )
    ...
    def _fill_entry(self, opp: dict, rel: pd.DataFrame, btc_usdt: float) -> None:
        ...
        if position_btc is None:
            sizing = self.equity.sizing_for_new_trade(self.open_opps)
            position_btc = None if sizing is None else sizing["position_btc"]
        if position_btc is None or float(position_btc) <= 0:
            opp["status"] = "REJECTED"
            opp["rejection_reason"] = "INSUFFICIENT_EQUITY"
            return
        position_btc = float(position_btc)
        if sizing is None:
            equity = float(self.equity.equity_btc)
            requested = float(self.equity.allocation_pct)
            actual = (position_btc / equity) if equity > 0 else 0.0
            sizing = {
                "position_btc": position_btc,
                "requested_allocation_pct": requested,
                "actual_allocation_pct": actual,
                "remaining_exposure_pct": self.equity.available_btc(self.open_opps) / equity if equity > 0 else 0.0,
                "exposure_limited": actual + 1e-12 < requested,
            }
        notional_usd = self.equity.notional_usd(position_btc, btc_usdt)
        ...
        intent = TradeIntent(
            ...
            side="LONG_ALT_BTC",
            decision_ts=opp.get("opened_ts"),
            position_btc=position_btc,
            requested_allocation_pct=float(sizing["requested_allocation_pct"]),
            actual_allocation_pct=float(sizing["actual_allocation_pct"]),
            remaining_exposure_pct=float(sizing["remaining_exposure_pct"]),
            exposure_limited=bool(sizing["exposure_limited"]),
            s_value=float(opp.get("S") or 0.0),
            regime=regime_info["regime"],
            selected_strategy_key=sk,
            selected_arm_label=pick.get("selected_arm_label"),
        )
        # PAPER OMS → PaperBroker → open_opportunity_legs (identical fill economics).
        open_n = sum(1 for o in self.open_opps if o.get("status") in ("OPEN", "PENDING_ENTRY"))
        reserved = self.equity.reserved_btc(self.open_opps)
        exposure = (reserved / self.equity.equity_btc) if self.equity.equity_btc > 0 else 0.0
        risk_ctx = RiskContext(
            open_position_count=open_n,
            current_exposure_pct=exposure,
            strategy_allows_new_entries=True,
            market_data_stale=False,
            account_data_stale=False,
        )
        cf_legs, sel_legs = self.oms.open_entry_legs(
            intent=intent,
            alt_btc_entry_mid=entry_mid,
            btc_usdt=btc_usdt,
            notional_usd=notional_usd,
            cf_specs=self.cf_specs,
            selected_spec=self.spec_by_key[sk],
            entry_ts=entry_ts,
            risk_ctx=risk_ctx,
        )
        leg_items: list[dict] = []
        ...
        opp.update({
            ...
            "notional_usd": notional_usd,
            "position_btc": position_btc,
            "allocation_pct": float(sizing["actual_allocation_pct"]),
            "requested_allocation_pct": float(sizing["requested_allocation_pct"]),
            "actual_allocation_pct": float(sizing["actual_allocation_pct"]),
            "remaining_exposure_pct": float(sizing["remaining_exposure_pct"]),
            "exposure_limited": bool(sizing["exposure_limited"]),
            "equity_btc_at_entry": self.equity.equity_btc,
            "leg_items": leg_items,
            "last_processed_ts": str(entry_ts),
            "regime": regime_info["regime"],
            "selected_arm_label": pick["selected_arm_label"],
            "selector_pick": pick,
            "execution_mode": self.oms.mode.value,
        })
        audit = {
        ...
        if self._notify_open:
            self.trade_notifier.trade_opened(
                opp,
                {**pick, "arm_label": "E"},
                equity_btc=self.equity.equity_btc,
                version=self.version,
        ...
        *,
        ...
        for r in ranked:
            ...
            if ep["trade_suggested"]:
                sizing = self.equity.sizing_for_new_trade(self.open_opps)
                if sizing is None or float(sizing["position_btc"]) <= 0:
                    rejection = "INSUFFICIENT_EQUITY"
                    ep = {"trade_suggested": False, "rejection_reason": rejection, "entry_classification": rejection}
                else:
                    ...
                    opp = {
                        ...
                        "position_btc": float(sizing["position_btc"]),
                        "allocation_pct": float(sizing["actual_allocation_pct"]),
                        "requested_allocation_pct": float(sizing["requested_allocation_pct"]),
                        "actual_allocation_pct": float(sizing["actual_allocation_pct"]),
                        "remaining_exposure_pct": float(sizing["remaining_exposure_pct"]),
                    ...
                    if opp.get("status") == "REJECTED":
                        rejection = opp.get("rejection_reason", "INSUFFICIENT_EQUITY")
                        ep = {"trade_suggested": False, "rejection_reason": rejection, "entry_classification": rejection}
                    else:
                        self.open_opps.append(opp)
    ...
    def _maybe_send_hourly_summary(self) -> None:
        if not self._notify_hourly:
            return
        ...
        if self._last_hourly_telegram_bucket == bucket:
            return
        try:
            from btcc.telegram.hourly_summary import compute_hourly_paper_stats

            stats = compute_hourly_paper_stats(
                legs=self.store.read_legs(),
                open_opps=self.open_opps,
                starting_equity_btc=self.equity.starting_equity_btc,
                current_equity_btc=self.equity.equity_btc,
                now=now,
            )
            ok = self.trade_notifier.hourly_summary(stats, version=self.version)
            self._last_hourly_telegram_bucket = bucket
            state = self.store.load_state()
            state["last_hourly_telegram_bucket"] = bucket
            state["last_hourly_telegram_utc"] = now.isoformat()
            state["last_hourly_telegram_sent"] = bool(ok)
            self.store.save_state(state)
            logger.info("Hourly paper Telegram summary sent=%s bucket=%s", ok, bucket)
        except Exception as e:
            logger.warning("Hourly paper Telegram summary failed: %s", e)
