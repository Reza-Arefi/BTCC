...
FIXED_T1_NO_LATE = "T1_NO_LATE"
FIXED_T1_LATE_FILTER = "T1_LATE_FILTER"
FIXED_T1_ARM_LABELS = (FIXED_T1_NO_LATE, FIXED_T1_LATE_FILTER)
CF_ARM_LABELS = tuple(f"T{i}" for i in range(1, 11))
CF_STRATEGY_KEYS = tuple(f"trail_{i}" for i in range(1, 11))


def fixed_t1_enabled(me: dict[str, Any]) -> bool:
    return bool((me.get("fixed_t1_baselines") or {}).get("enabled", False))


def selector_arm_labels(sim: dict[str, Any] | None = None) -> tuple[str, ...]:
    """E-memory variants plus optional fixed T1 baseline arms."""
    sim = sim or load_selector_memory_config()
    me = sim.get("selector_memory_experiment") or {}
    arms = list(MEMORY_ARM_LABELS)
    if fixed_t1_enabled(me):
        ft = me.get("fixed_t1_baselines") or {}
        arms.extend([
            str(ft.get("no_late_arm", FIXED_T1_NO_LATE)),
            str(ft.get("late_filter_arm", FIXED_T1_LATE_FILTER)),
        ])
    return tuple(arms)
...
def validate_selector_memory(sim: dict[str, Any]) -> list[str]:
    ...
    if fixed_t1_enabled(me):
        ft = me.get("fixed_t1_baselines") or {}
        sk = str(ft.get("strategy_key", "trail_1"))
        if sk not in strategies:
            errors.append(f"fixed_t1_baselines.strategy_key {sk} missing from strategies")
...
def pre_run_memory_summary(sim: dict[str, Any] | None = None) -> str:
    ...
    if fixed_t1_enabled(me):
        ...
        lines += [
            "",
            "FIXED T1 BASELINES:",
            f"  {ft.get('no_late_arm', FIXED_T1_NO_LATE)}: T1 exit, no late-entry rejection",
            f"  {ft.get('late_filter_arm', FIXED_T1_LATE_FILTER)}: T1 exit, late-entry filter ON",
        ]
    lines += [
        "",
